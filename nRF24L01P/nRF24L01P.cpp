...
void nRF24L01P::setRegister(int regAddress, int regData)
{
    ...
    // Write the data in regData into the register at regAddress.
    // The use the spi_, nCS_, and ce_ member variables included in the nRF24L01P class.
    // Only use DigitalOut & SPI MBed functions when implementing this function.
    // TODO: END HERE
    ...
    int cn = (_NRF24L01P_SPI_CMD_WR_REG | (regAddress & _NRF24L01P_REG_ADDRESS_MASK)); //(0x20 | (regAddress & 0x1F))
    ...
    ce_ = originalCe;
    wait_us(_NRF24L01P_TIMING_Tpece2csn_us); //wait 4us
}
...
int nRF24L01P::getRegister(int regAddress)
{
    ...
    // Return the data in the register at regAddress.
    // The use the spi_, nCS_, and ce_ member variables included in the nRF24L01P class.
    // Only use DigitalOut & SPI MBed functions when implementing this function.
    // TODO: END HERE

    // do cn masking like above
    int cn = (_NRF24L01P_SPI_CMD_RD_REG | (regAddress & _NRF24L01P_REG_ADDRESS_MASK)); //(0x00 | (regAddress & 0x1F))

    nCS_ = 0;

    int status = spi_.write(cn);

    int dn = spi_.write(_NRF24L01P_SPI_CMD_NOP); //write(0xFF), write will return what is in the read buffer and store into dn
...
void nRF24L01P::enableAutoAcknowledge(int pipe)
{
    ...
    // TODO: START HERE
    // This function enables the AA bit in the Enable AutoAck register for the pipe number passed into this function.
    // TODO: END HERE

    if ((pipe < NRF24L01P_PIPE_P0) || (pipe > NRF24L01P_PIPE_P5))
    { //check pipe is within limit 0 <= pipe <= 5

        error("nRF24L01P: Invalid Enable AutoAcknowledge pipe number %d\r\n", pipe);
        return;
    }

    int enAA = getRegister(_NRF24L01P_REG_EN_AA); //MUST DO

    enAA |= (1 << (pipe - NRF24L01P_PIPE_P0)); //enAA |= (1 << pipe), MUST USE |=

    setRegister(_NRF24L01P_REG_EN_AA, enAA); //MUST DO
}
...
void nRF24L01P::setRfFrequency(int frequency)
{
    ...
    // TODO: START HERE
    // This function writes the difference between the frequency argument and the minimum output frequency
    // of the NRF24L01P into the RF Channel Register.
    // Remember to make sure that the frequency argument is valid.
    // TODO: END HERE
    if ((frequency < NRF24L01P_MIN_RF_FREQUENCY) || (frequency > NRF24L01P_MAX_RF_FREQUENCY))
    { //check valid RFFrequency
        //be between 2525 & 2400

        error("nRF24L01P: Invalid RF Frequency setting %d\r\n", frequency);
        return;
    }

    int channel = (frequency - NRF24L01P_MIN_RF_FREQUENCY) & 0x7F; //subtract with min freq, keep only LSB 7 bits
                                                                   //technically can never set 8th bit to 1
...
void nRF24L01P::setCrcWidth(int width)
{
    ...
    // TODO: START HERE
    // This function sets the CRC width in the Config Register.
    // Valid Crc widths are 0, 1, or 2 bytes.
    // TODO: END HERE

    int config = getRegister(_NRF24L01P_REG_CONFIG) & ~_NRF24L01P_CONFIG_CRC_MASK; //idk why inverse lol

    switch (width)
    {

    case NRF24L01P_CRC_NONE:
        config |= _NRF24L01P_CONFIG_CRC_NONE; //Bit 4 = 0
        break;

    case NRF24L01P_CRC_8_BIT:
        config |= _NRF24L01P_CONFIG_CRC_8BIT; //Bit 4 = 1, Bit 3 = 0
        break;

    case NRF24L01P_CRC_16_BIT:
        config |= _NRF24L01P_CONFIG_CRC_16BIT; //Bit 4 = 1, Bit 3 = 1
        break;
                     ...
                     PinName irq) : spi_(mosi, miso, sck), nCS_(csn), ce_(ce), nIRQ_(irq)
{
    ...
    // TODO: START HERE
    // Use the format(2) SPI command to configure the Nucleo to match the NRF24L01P's polarity and phase
    // TODO: END HERE
...
void nRF24L01P::setReceiveMode(void)
{
    ...
    if (_NRF24L01P_MODE_POWER_DOWN == mode)
        powerUp();
